...
final class DeliveringJwtSigner
{
    ...
    private static function sign(array $claims, string $privateKey, int|string $algorithm, bool $ecdsa, array $headers): string
    {
        ...
        if (false === $key) {
            throw new DeliveringPermanentTransportException('Push provider private key is invalid.');
        }
        $signature = '';
        if (!openssl_sign($input, $signature, $key, $algorithm)) {
            throw new DeliveringPermanentTransportException('Push provider JWT signing failed.');
        }
        if ($ecdsa) {
    ...
    private static function ecdsaDerToJose(string $der, int $partLength): string
    {
        ...
        if (0x30 !== self::byte($der, $offset++)) {
            throw new DeliveringPermanentTransportException('APNs ES256 signature is not a DER sequence.');
        }
    ...
    private static function readInteger(string $der, int &$offset): string
    {
        if (0x02 !== self::byte($der, $offset++)) {
            throw new DeliveringPermanentTransportException('APNs ES256 signature contains an invalid integer.');
        }
    ...
    private static function readLength(string $der, int &$offset): int
    {
        ...
        if ($octets < 1 || $octets > 4) {
            throw new DeliveringPermanentTransportException('Push provider DER length is invalid.');
        }
        $length = 0;
        for ($i = 0; $i < $octets; ++$i) {
    ...
    private static function normalizeInteger(string $value, int $length): string
    {
        ...
        if (strlen($value) > $length) {
            throw new DeliveringPermanentTransportException('Push provider ECDSA signature integer is too large.');
        }
    ...
    private static function byte(string $value, int $offset): int
    {
        if ($offset >= strlen($value)) {
            throw new DeliveringPermanentTransportException('Push provider DER signature is truncated.');
        }
